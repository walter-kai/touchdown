...
import SummaryView from '@/views/espn/summary/Summary';
...
const GameDetail: React.FC<GameDetailProps> = ({ activeTab, onTabChange, onPresetChange, onGameStatusChange, onRegisterTabClick }) => {
  ...
  // Reset playsLoaded when gameId changes so plays can be fetched for the new game
  useEffect(() => {
    debugLog(`GameId changed to ${gameId}, resetting plays state`);
    setPlaysLoaded(false);
    setPlayLog([]);
  }, [gameId]);
  ...
  const refreshGameAndPlays = useCallback(async () => {
    ...
    try {
      ...
      // cache latest pulls for quick resume (skip if quota exceeded)
      try {
  ...
  useEffect(() => {
    const loadPreviousPlays = async () => {
      if (!gameId || playsLoaded || !event) return; // Wait for event to be loaded
      ...
      try {
        ...
        // Cache the data (skip if quota exceeded)
        try {
  ...
  const mergeLatestPlay = (currentEvent: Event | undefined, previousEvent: Event | null) => {
    ...
    if (currentPlayText !== previousPlayText) {
      ...
      setPlayLog(prev => {
        ...
        if (!isDuplicate) {
          const possession = (comp.situation?.possession as any) || undefined;
          const newPlay: PlayNfl = {
            ...
            possession: possession && typeof possession === 'object' && 'id' in possession ? possession.id : possession,
  ...
  const getGameData = async (gid: string): Promise<{ game?: Event; usedSummaryApi: boolean }> => {
    ...
    const actualGameId = gid === 'test' ? testGameId : gid;
    
    // Try Summary API first to get full game context and determine status
    try {
      debugLog(`Fetching game data via Summary API for game ${actualGameId}...`);
      
      const summaryResponse = await axios.get<Summary>(
        getSummaryUrl(league, actualGameId)
      );
      setSummary(summaryResponse.data);
      usedSummaryApi = true;
      
      if (summaryResponse.data.header) {
        const headerGame = summaryResponse.data.header as unknown as Event;
        const gameStatus = headerGame?.competitions?.[0]?.status?.type?.state;
        
        // For LIVE games, also fetch from scoreboard to get real-time scores
        // Summary API's header may have stale scores for live games
        if (gameStatus === 'in') {
          debugLog(`🔴 Game is LIVE - fetching from scoreboard for real-time scores...`);
          try {
            const scoreboardResponse = await axios.get<ScoreboardResponse>(
              getScoreboardUrl(league)
            );
            const liveGame = scoreboardResponse.data.events?.find(e => e.id === actualGameId);
            
            if (liveGame) {
              debugLog(`✅ Got live scores from scoreboard API`);
              game = liveGame; // Use scoreboard data for live games (has real-time scores)
            } else {
              debugLog(`⚠️ Game not in scoreboard, using summary header`);
              game = headerGame; // Fall back to summary if not in scoreboard
            }
          } catch (scoreboardErr) {
            console.error('Error fetching live scoreboard:', scoreboardErr);
            game = headerGame; // Fall back to summary data
          }
        } else {
          // For pre-game or completed games, summary is fine
          debugLog(`✅ Successfully loaded game data from Summary API (status: ${gameStatus})`);
          game = headerGame;
        }
      }
    } catch (summaryErr: any) {
      if (summaryErr?.response?.status === 404) {
        debugLog(`⚠️ Summary not found for game ${gid} (404) - trying scoreboard as fallback`);
      } else {
        console.error('Error fetching summary data:', summaryErr);
      }
      
      // Fallback to scoreboard API only if summary fails completely
      try {
        const scoreboardResponse = await axios.get<ScoreboardResponse>(
          getScoreboardUrl(league)
        );
        game = scoreboardResponse.data.events?.find(e => e.id === actualGameId);
        
        if (game) {
          debugLog(`✅ Found game in scoreboard API as fallback`);
        }
      } catch (scoreboardErr) {
        console.error('Error fetching scoreboard data:', scoreboardErr);
      }
    }
