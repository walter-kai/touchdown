...
export const gettingStartedSteps = [
  {
    ...
    detail: (
      <div className="bg-bg-dark p-4 rounded-lg border border-neon-cyan/20 ">
        <div className="flex text-neon-cyan font-semibold bg-bg-darker p-3 rounded border border-neon-cyan/10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="flex text-neon-cyan font-semibold bg-bg-darker p-3 rounded border border-neon-cyan/10">
            ...
            <h4 className="mb-2 gap-2">
              Bot Garage
            <p className="text-text-light text-sm">Customize a bot tailored to your preferences and store them for later use.</p>
            </h4>
          </div>
          <div className="flex text-neon-cyan font-semibold bg-bg-darker p-3 rounded border border-neon-cyan/10">
            <img src='icons/building-garage.svg' alt='Bot Garage' className='w-24 h-24 inline-block' />
      ...
      <div className="bg-bg-dark p-4 rounded-lg border border-neon-cyan/20">
        <div className="flex items-center gap-3 mb-3">
          <img
            ...
            className="w-8 h-8"
          />
          <span className="text-neon-cyan font-semibold">Uniswap Integration</span>
        </div>
        <p className="text-text-light mb-3">
          Trade on the most popular DEX with access to:
        </p>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
          <span className="bg-bg-darker px-2 py-1 rounded text-[#b8eaff]">🚀 ETH/USDC</span>
          <span className="bg-bg-darker px-2 py-1 rounded text-[#b8eaff]">🐸 PEPE/ETH</span>
          <span className="bg-bg-darker px-2 py-1 rounded text-[#b8eaff]">🐕 SHIB/ETH</span>
          <span className="bg-bg-darker px-2 py-1 rounded text-[#b8eaff]">⚡ WBTC/ETH</span>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            <div className="relative group">
              <button className="bg-bg-darker text-neon-cyan border border-neon-cyan/30 px-3 py-1 rounded-md text-sm font-semibold hover:bg-neon-cyan/20 transition-colors flex items-center gap-1">
                <span>🔋</span> Recharge
              </button>
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 bg-bg-darker text-white text-xs rounded p-2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
...
const HowItWorks: React.FC = () => {
  ...
  return (
    <div 
      ref={sectionRef}
      id="getting-started" 
      className="bg-bg-darker/70 border border-neon-cyan/30 rounded-xl shadow-lg p-8 mb-12 hover:shadow-[0_0_32px_rgba(0,255,231,0.2)] transition-all duration-300"
    >
      <h2 className="text-3xl font-bold text-neon-cyan mb-8 text-center drop-shadow-[0_0_8px_#00ffe7] opacity-0 animate-fade-in-up">
        ...
        {gettingStartedSteps.map((step, index) => (
          <React.Fragment key={step.step}>
            <div 
              ...
              className={`w-full max-w-3xl p-6 bg-bg-dark/50 rounded-lg border border-neon-cyan/20 hover:border-neon-cyan/40 transition-all duration-700 mb-4 ${
            ...
            >
              <div className="flex items-start gap-6">
                <div className={`flex-shrink-0 transition-all duration-500 ${
                  ...
                  <div className="w-16 h-16 bg-neon-cyan text-bg-dark rounded-full flex items-center justify-center text-2xl font-bold shadow-[0_0_16px_#00ffe7]">
                    {step.step}
                  </div>
                </div>
                <div className="flex-1">
                  <div className={`flex items-center gap-3 mb-3 transition-all duration-500 ${
                    ...
                    <div className="text-2xl text-neon-cyan">{step.icon}</div>
                    <h3 className="text-xl font-bold text-neon-cyan">{step.title}</h3>
                  </div>
                  <p className={`text-neon-pink mb-4 text-lg transition-all duration-500 ${
            ...
            {index < gettingStartedSteps.length - 1 && (
              <div className={`transition-all duration-500 ${visibleSteps[index] ? 'opacity-100' : 'opacity-0'}`} style={{ transitionDelay: `${index * 200 + 100}ms` }}>
                <div className="w-1 h-12 bg-neon-cyan/30 mx-auto my-4"></div>
              </div>
            )}
          </React.Fragment>
        ))}
      ...
      {/* Strategy Slider */}
      <div className="bg-bg-dark/70 p-6 rounded-lg border border-neon-cyan/30 mt-8 opacity-0 animate-fade-in-scale" style={{ animationDelay: '0.3s' }}>
